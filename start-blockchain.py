...
def run(args):
    print('start-blockchain.py:', args)
    logFile.write(args + '\n')
    if subprocess.call(args, shell=True):
        print('start-blockchain.py: exiting because of error')
        sys.exit(1)
...
def startNode(nodeIndex, account):
    ...
    cmd = (
        ...
        '    --p2p-peer-address 167.172.99.207:' + str(9000) +
        otherOpts)
...
def allocateFunds(b, e):
    accounts[0]['funds'] = 50_000_000 * 10_000
    accounts[1]['funds'] = 27_000_000 * 10_000
    return 77_000_000 * 10_000
...
def createStakedAccounts(b, e):
    ...
    print('createStakedAccounts, Range %d to %d' % (b,e))
    for i in range(b, e):
        print('Index = %d' % i)
        a = accounts[i]
        funds = a['funds']
...
def vote(b, e):
    retry(args.cleos + 'system voteproducer prods ' + '4chanholder1' + ' ' + '4chanprod1')
...
def stepCreateTokens():
    run(args.cleos + 'push action eosio.token create \'["eosio", "77000000.0000 %s"]\' -p eosio.token' % (args.symbol))
...
def stepInitSystemContract():
    run(args.cleos + 'push action eosio init' + jsonArg(['0', '4,CHAN']) + '-p eosio@active')
    sleep(1)
...
parser.add_argument('--contracts-dir', metavar='', help="Path to latest contracts directory", default='eosio.contracts-1.9.x/contracts/')
parser.add_argument('--old-contracts-dir', metavar='', help="Path to 1.8.x contracts directory", default='eosio.contracts-1.8.x/contracts/')
parser.add_argument('--nodes-dir', metavar='', help="Path to nodes directory", default='./nodes/')
parser.add_argument('--genesis', metavar='', help="Path to genesis.json", default="./genesis.json")
parser.add_argument('--wallet-dir', metavar='', help="Path to wallet directory", default='./wallet/')
parser.add_argument('--log-path', metavar='', help="Path to log file", default='./output.log')
parser.add_argument('--symbol', metavar='', help="The eosio.system symbol", default='CHAN')
...
parser.add_argument('--num-senders', metavar='', help="Number of users to transfer funds randomly", type=int, default=3)
...
for (flag, command, function, inAll, help) in commands:
    if getattr(args, command) or inAll and args.all:
        if function:
            ...
            function()
if not haveCommand:
    print('start-blockchain.py: Tell me what to do. -a does almost everything. -h shows options.')